...
class ClassGenerator extends GeneratorAbstract
{
    ...
    public static function convertValue( $value )
    {
        ...
        if ( mb_strpos( $value, '"' ) === 0 || mb_strpos( $value, "'" ) === 0 || mb_strpos( $value, '[' ) === 0 || mb_strpos( $value, 'array' ) === 0 || mb_strpos( $value, '::' ) !== false ) {
            return $value;
        }
    ...
    public static function paramsFromString( $params )
    {
        ...
        $continue = true;
        $rand = 'foo' . random_int( 1, 20000 ) . random_int( 1, 20000 ) . random_int( 1, 30000 ) . md5( time() + rand( 1, 10000 ) );
        $newParams = [];
        $class = <<<EOF
class {$rand} {
