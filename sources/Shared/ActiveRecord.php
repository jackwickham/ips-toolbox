...
trait ActiveRecord
{
    ...
    public static function all( array $config = [] ): ActiveRecordIterator
    {
        ...
        $sql = Db::i()->select( $config[ 'columns' ] ?? static::$databaseTable . '.*', static::$databaseTable, $config[ 'where' ] ?? null, $config[ 'order' ] ?? null, $config[ 'limit' ] ?? null, $config[ 'group' ] ?? null, $config[ 'having' ] ?? null, $config[ 'flags' ] ?? 0 );
    ...
    public function getData( bool $fresh = true, bool $prefix = false ): array
    {
        ...
        if ( $fresh === true ) {
            return Db::i()->select( '*', static::$databaseTable, [
