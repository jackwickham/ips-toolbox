...
trait SchemaBuilder
{
     ...
     *
     * @throws RuntimeException|\IPS\Db\Exception
     */
    public function _buildSchemaFile( $table, Application $application )
    {
        try {
            ...
            if ( !is_dir( $path . 'setup/upg_working/' ) ) {
                if ( !mkdir( $path . 'setup/upg_working', \IPS\IPS_FOLDER_PERMISSION, \true ) && !is_dir( $path . 'setup/upg_working' ) ) {
                    throw new RuntimeException( sprintf( 'Directory "%s" was not created', $path . 'setup/upg_working' ) );
                }
                chmod( $path . 'setup/upg_working/', \IPS\IPS_FOLDER_PERMISSION );
            }

            $file = $path . 'setup/upg_working/queries.json';
            $queriesJson = [];
            if ( file_exists( $file ) ) {
                $queriesJson = json_decode( file_get_contents( $file ), \true );
            }
            ...
            $schema = [];
            if ( file_exists( $file ) ) {
                $schema = json_decode( file_get_contents( $file ), \true );
            }
